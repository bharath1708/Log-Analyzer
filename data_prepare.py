...
def gen_aws_secret():
    return random_string(40, string.ascii_letters + string.digits + "/+=")
...
def gen_secret_in_normal_text():
    ...
    return f"{fake.sentence()} {secret} {fake.sentence()}"

def gen_non_secret_in_normal_text():
    fake = Faker()
    gen = random.choice([gen_normal_text, gen_noisy_non_secret, gen_hard_negative])
    return f"{fake.sentence()} {gen()} {fake.sentence()}"
...
if __name__ == "__main__":
    df = build_dataset(n=200000, ratio=0.12)
    print(df.sample(20))
